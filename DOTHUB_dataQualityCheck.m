...
tmp = length(find((SDtmp.MeasListAct(1:end/nWavs)==1 & dists'>rng(1) & dists'<rng(2)))) / length(find(dists'>rng(1) & dists'<rng(2)));
...
tmp = length(find((SDtmp.MeasListAct(1:end/nWavs)==1 & dists'>rng(1) & dists'<rng(2)))) / length(find(dists'>rng(1) & dists'<rng(2)));
fprintf(['Percentage of good channels in range [' num2str(rng(1)) ' ' num2str(rng(2)) '] = ' num2str(tmp*100,'%0.1f') '%% \n']);
fprintf(['Total number of good fNIRS channels = ' num2str(sum(SDtmp.MeasListAct(1:end/nWavs))) '/' num2str(length(SDtmp.MeasListAct(1:end/nWavs))) '\n']);
diary off;
