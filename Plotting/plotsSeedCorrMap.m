...
function [Fig1, corrcoeffs]=DOTHUB_selectseedCorrMap(dotimgFileName,gmSurfaceMesh,GMSensitivityMask, size);
...
% size                      % size of seed region (in mm), reccomended is
                            % 1-6
% Output: 
% Fig 1                     % correlation map (pearson's R)
% corrcoefs                 % correlation values for each node
...
figure; DOTHUB_plotSurfaceImage(gmSurfaceMesh,image);
...
% Plot the correlation map
 Fig1=figure; DOTHUB_plotSurfaceImage(gmSurfaceMesh,corrcoefs);
