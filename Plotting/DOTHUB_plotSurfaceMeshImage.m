function DOTHUB_plotSurfaceMeshImage(mesh,intensity,viewAng,shadingtype,cmap)
...
nodes = mesh.node;
face  = mesh.face;
h = trisurf(face(:,1:3), nodes(:,1), nodes(:,2), nodes(:,3),intensity,'EdgeColor',[0.8 0.8 0.8],'EdgeAlpha',1);
shading(shadingtype);
set(h,'diffusestrength',.7,'specularstrength',.2,'ambientstrength',.2);
