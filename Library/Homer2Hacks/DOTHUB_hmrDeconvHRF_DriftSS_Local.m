...
% DOTHUB_hmrDeconvHRF_DriftSS_Local(y, s, t, SD, Aaux, tIncAuto, trange, glmSolveMethod, idxBasis, paramsBasis, rhoSD_ssThresh, flagSSmethod, driftOrder, flagMotionCorrect )
%
% UI NAME: 
% GLM_HRF_Drift_SS
%
...
%              channels around ( < rhoSD_ssThresh) to the long
%              source-detector separation channel
%            4 if performed with the average of the short separation
%              channels around ( < rhoSD_ssThresh) to the location of the
...
%
% LOG
% 06/2019 (EEVR, UCL) Updated to regress two new types of regressors:
...
function [yavg, yavgstd, tHRF, nTrials, ynew, yresid, ysum2, beta, yR] = DOTHUB_hmrDeconvHRF_DriftSS_Local(y, s, t, SD, Aaux, tIncAuto, trange, glmSolveMethod, idxBasis, paramsBasis, rhoSD_ssThresh, flagSSmethod, driftOrder, flagMotionCorrect )
...
lstSS = lst(rhoSD<=rhoSD_ssThresh & mlAct(lst)==1);

if isempty(lstSS)
    disp('There are no short separation channels in this probe...performing regular deconvolution.');
    iNearestSS=[];
    mlSSlst = 0;
else
    ...
    elseif flagSSmethod==3 % use local average of many SS around channel
        iNearestSS = zeros(length(lst),100); % Enough number of column
        for iML = 1:length(lst)
            rho = sum((ones(length(lstSS),1)*posM(iML,:) - posM(lstSS,:)).^2,2).^0.5;        
            ii = find(rho <= rhoSD_ssThresh);
            if ~isempty(ii)
                iNearestSS(iML,(1:length(ii))) = lstSS(ii);
            else
                [~,ii] = min(rho);
                iNearestSS(iML,1) = lstSS(ii);
            end
        end            
        iNearestSS = iNearestSS(:,sum(iNearestSS,1)>0);
        Num_iNearestSS = sum((iNearestSS > 0),2);
        
    elseif flagSSmethod==4 % use local average of many SS around source and detector
        iNearestSS = zeros(length(lst),100); % Enough number of column
        for iML = 1:length(lst)
            
            % SOURCE SIDE------------------------------------------------
            % Find the short separation channels closer (<= rhoSD_ssThresh)
            % to the location of the source of the long source-detector
            % distance channel
            Src = SD.SrcPos(ml(lst(iML),1),:);
            rho = sum((ones(length(lstSS),1)*Src - posM(lstSS,:)).^2,2).^0.5;        
            iis = find(rho <= rhoSD_ssThresh);
            if ~isempty(iis)
                iNearestSS(iML,(1:length(iis))) = lstSS(iis); shift = length(iis);
            else
                [~,iis] = min(rho);
                iNearestSS(iML,1) = lstSS(iis); shift = 1;
            end
            
            % DETECTOR SIDE------------------------------------------------
            % Find the short separation channels closer (<= rhoSD_ssThresh)
            % to the location of the detector of the long source-detector
            % distance channel
            Det = SD.DetPos(ml(lst(iML),2),:);
            rho = sum((ones(length(lstSS),1)*Det - posM(lstSS,:)).^2,2).^0.5;        
            iid = find(rho <= rhoSD_ssThresh);
            if ~isempty(iid)
                iNearestSS(iML,(1:length(iid)) + shift) = lstSS(iid);
            else
                [~,iid] = min(rho);
                iNearestSS(iML,1 + shift) = lstSS(iid);
            end
        end
        % Identify repeated SS channels and delete them from the list of
        % short-separation regressors
        uniq_iNearestSS = zeros(  size(iNearestSS));
        for i = 1:size(iNearestSS,1)
            u = setdiff(  unique(  iNearestSS(i,:)),0);
            uniq_iNearestSS(i,1:length(u)) = u;
        end
        % Clear columns with no SS channels
        iNearestSS = uniq_iNearestSS(:,sum(uniq_iNearestSS,1)>0);
        % Determine SS channel groups
        Num_iNearestSS = sum((iNearestSS > 0),2);        
...
for iCond = 1:nCond
    ...
    lstp=(lstT+nPre)>=1 & (lstT+nPost)<=nTpts;
    lst=lstT(lstp);
...
for conc=1:2 %only HbO and HbR
  ...
  % loop over number of short separation groups(dependent one channel, two channels, etc)
  for Col_iNearestSS = 1:size(iNearestSS,2)    
    % loop over short separation groups
    if exist('Num_iNearestSS','var') == 1
        mlSSlst = unique(iNearestSS(Num_iNearestSS == Col_iNearestSS,1:Col_iNearestSS),'rows');
    end
    for iSS = 1:size(mlSSlst,1)
        ...
        elseif flagSSmethod==2
            ...
            At = [A(:,:,conc) Ass];
        elseif flagSSmethod==3
            lstML = find(Num_iNearestSS == Col_iNearestSS & prod(iNearestSS(:,1:Col_iNearestSS)==mlSSlst(iSS,1:Col_iNearestSS),2) & mlAct(lstMLtmp)==1);
            %disp([lstML iNearestSS(lstML,:)])
            Ass = mean(y(:,conc,mlSSlst(iSS,1:Col_iNearestSS)),3);
            At = [A(:,:,conc) Ass];
        elseif flagSSmethod==4
            lstML = find(Num_iNearestSS == Col_iNearestSS & prod(iNearestSS(:,1:Col_iNearestSS)==mlSSlst(iSS,1:Col_iNearestSS),2) & mlAct(lstMLtmp)==1);
            %disp([lstML iNearestSS(lstML,:)])
            Ass = mean(y(:,conc,mlSSlst(iSS,1:Col_iNearestSS)),3);
            At = [A(:,:,conc) Ass];
        ...
        if ~isempty(lstML)
            ...
            if glmSolveMethod==1 % ~flagUseTed
                ...
                if RCOND > 1e-15
                    ...
                    warning(['Warning: Matrix is close to singular or badly scaled. Results may be inaccurate. RCOND = ',num2str(RCOND)])
                    sv = svd(ATA);
                    lambda = 0.01;
                    pinvA=(ATA+lambda*max(sv)*eye(size(ATA)))\At(lstInc,:)';
                    warning(['Trying Tikhonov regularization with regularization lambda = ',num2str(lambda)])
                end
                foo = [];
  ...
  end % end loop on NUMBER of short separation groups (dependent one channel, two channels, etc)
