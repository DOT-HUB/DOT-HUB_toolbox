...
SD_3Dmesh = SD_3D;
for i = 1:SD_3D.nSrcs
    tmp = DOTHUB_nearestNode(SD_3D.SrcPos(i,:),scalpSurfaceMeshReg.node(:,1:3));
    SD_3Dmesh.SrcPos(i,:) = DOTHUB_nearestNode(tmp,headVolumeMeshReg.node(:,1:3));
end
for i = 1:SD_3D.nDets
    tmp = DOTHUB_nearestNode(SD_3D.DetPos(i,:),scalpSurfaceMeshReg.node(:,1:3));
    SD_3Dmesh.DetPos(i,:) = DOTHUB_nearestNode(tmp,headVolumeMeshReg.node(:,1:3));
end
